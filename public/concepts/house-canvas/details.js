...
</section><section class="player-library" aria-label="Favourite playlists"><div class="device-list favourite-playlists">${favouritePlaylists.map((title, i) => button(title, 'favourite', `<span class="playlist-art"><span class="playlist-art-fallback" aria-hidden="true"><span>♫</span><small>${esc(title)}</small></span>${favouriteArtwork[title] ? `<img class="playlist-cover" src="${esc(favouriteArtwork[title])}" alt="" loading="lazy" referrerpolicy="no-referrer">` : ''}<span class="playlist-play">${icon(state.music.playlist === title ? 'check' : 'play')}</span></span>`, `class="device-item" data-value="${i}" data-focus-key="favourite-${i}" aria-pressed="${state.music.playlist === title}"`)).join('')}</div></section></div>`
        }`,
      };
    }
    case 'weather': {
