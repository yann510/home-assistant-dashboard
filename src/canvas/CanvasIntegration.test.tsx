...
it('keeps All lights and its scroll position mounted while settings open and Escape restores the same trigger', () => {
  ...
  render(<CanvasDashboard />);
  fireEvent.click(screen.getByRole('button', { name: 'All lights' }));
  const dialog = screen.getByRole('dialog', { name: 'All lights' });
  const body = dialog.querySelector('.canvas-dialog__body')!;
  body.scrollTop = 420;
  const room = screen.getByRole('region', { name: 'Gym lights' });
  const power = within(room).getByRole('button', { name: 'Turn off Gym lights' });
  const settings = within(room).getByRole('button', { name: 'Gym settings' });
  fireEvent.click(settings);
  expect(screen.getAllByRole('dialog')).toEqual([dialog]);
  expect(body.scrollTop).toBe(420);
  expect(power.isConnected).toBe(true);
  expect(power.closest('[inert]')).toBeTruthy();
