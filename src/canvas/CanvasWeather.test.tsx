...
it('keeps missing readings unavailable, then retries a failed subscription', async () => {
  ...
  expect(within(row).getByText('Wind unavailable')).toBeTruthy();
...
it('omits invalid optional readings and never converts precipitation amount into chance', () => {
  ...
  expect(screen.queryByText(/^Wind \d/)).toBeNull();
...
it('shows actual provider wind and precipitation amount with their own units', () => {
  ...
  const row = screen.getByRole('listitem');
  const accessibleWind = screen.getByText('Wind 11.5 km/h');
  const windCell = accessibleWind.parentElement!;
  expect(windCell.parentElement).toBe(row);
  expect(accessibleWind.className).toBe('canvas-weather__sr-only');
  expect(accessibleWind.hasAttribute('aria-hidden')).toBe(false);
  expect(within(windCell).getByText('11.5').getAttribute('aria-hidden')).toBe('true');
