  ...
  return (
    <section className='canvas-blinds' aria-label='Blinds'>
      <div className='canvas-blinds__heading'>
        <h2>Blinds</h2>
        <span className='canvas-blinds__caption'>Choose rooms</span>
      </div>
          ...
          <button
            ...
            aria-label={
              action === 'stop' && movingRooms.length
                ? `Stop moving blinds in ${movingRooms.map(label).join(', ')}`
            ...
            disabled={
              ...
              pending[action] ||
              (action !== 'stop' && (pending.open || pending.close))
            }
      ...
      <p className='canvas-blinds__caption' role='status'>
        {!connected
          ? 'Home Assistant is disconnected. Reconnect to control blinds.'
          : selection.length
            ? `${selection.map(label).join(' + ')} selected`
            : 'Select one or more rooms above'}
      </p>
      {movingRooms.length > 0 && (
        <p className='canvas-blinds__caption'>
          Movement request pending for {movingRooms.map(label).join(' + ')}. Stop targets these rooms.
        </p>
      )}
      {actions.map(
        action =>
          results[action] && (
            <div key={action} className='canvas-blinds__results' role='group' aria-label={`${actionLabel(action)} blind command results`}>
