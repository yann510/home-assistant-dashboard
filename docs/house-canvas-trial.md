...
Verified at **2026-09-23T19:48:55.441Z**. The independently reviewed player-status fix passed **344 full-suite tests**, focused music tests, changed-file ESLint, project type checking and an explicit trial-base build. See [UX polish verification](house-canvas-ux-polish.md).

- `npm run deploy:canvas-trial -- b23b6578d7bab9162815a5e2d8eba425e15d28c4` exited 0 after its secret scan, staged SHA-256 verification and promotion. Remote release-info reports that exact source/release ID.
- All **82 manifest files** match their remote SHA-256 hashes and LAN HTTP responses (HTTP 200). All **73 current build files** match local `dist`; **8 previous hashed assets** remain available and verified, alongside generated release metadata.
...
- Previous release `287c1bc5c5521b64b58282cd35540fb2f05275e1` is preserved in `/homeassistant/www/canvas-trial-previous`; all **80 previous manifest files** were re-read and verified. `npm run rollback:canvas-trial` can restore it while retaining the new hashed assets for open clients. Rollback was not executed during this verification.
