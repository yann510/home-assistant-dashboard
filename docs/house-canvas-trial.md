...
Deployed source commit and release ID: `bb788a1df49d8ef0ac10ca5d3e4cce3d9af1b6aa` on `codex/quiet-home-poc`. Previous release `b23b6578d7bab9162815a5e2d8eba425e15d28c4` is retained for rollback. Source publication to `origin/main` is separate from deployment. Canvas remains optional; adopting it as the default remains a later decision.
...
Verified at **2026-09-23T19:54:43.301Z**. The independently reviewed compact pulse CSS fix passed scoped ESLint, project type checking and an explicit trial-base build. No full-suite rerun was needed for this CSS-only production correction; the preceding suite passed **344 tests**. See [UX polish verification](house-canvas-ux-polish.md).

- `npm run deploy:canvas-trial -- bb788a1df49d8ef0ac10ca5d3e4cce3d9af1b6aa` exited 0 after its secret scan, staged SHA-256 verification and promotion. Remote release-info reports that exact source/release ID.
- All **84 manifest files** match their remote SHA-256 hashes and LAN HTTP responses (HTTP 200). All **73 current build files** match local `dist`; **10 previous hashed assets** remain available and verified, alongside generated release metadata.
...
- Previous release `b23b6578d7bab9162815a5e2d8eba425e15d28c4` is preserved in `/homeassistant/www/canvas-trial-previous`; all **82 previous manifest files** were re-read and verified. `npm run rollback:canvas-trial` can restore it while retaining the new hashed assets for open clients. Rollback was not executed during this verification.
