...
## Compact hourly wind layout

Reviewed source `0e3526157c5f878b520b3f6e4f24568f87b58ec7` puts hourly wind in the main five-column forecast grid, with its unit shown once in the header. Each reading also has real visually hidden text containing wind/value/unit or an unavailable description; the visible numeric duplicate is hidden from assistive technology. Optional feels-like stays beneath temperature. Phone conditions retain icons and accessible text while hiding the visible description; daily columns and precipitation semantics are unchanged.

All 14 focused weather tests, scoped ESLint, whitespace checks and the final explicit trial build/typecheck passed. Independent re-review approved the accessibility correction. The root checked the 960 × 600 and 375 px layouts with met-shaped and optional feels-like data: headings align, normal rows remain 62 px and there is no overflow; daily view remains readable. The preceding full-suite result is 349 tests, not a new full-suite run for this change. Provider limitations and pending live/physical acceptance remain unchanged. No physical commands were issued.
